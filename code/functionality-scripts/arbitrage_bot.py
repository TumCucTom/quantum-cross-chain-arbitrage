...
def get_curve_pool_address(token0: str, token1: str) -> str:
    ...
    try:
        ...
        # ✅ Call Curve factory contract correctly
...
async def main():
    ...
    for pair in FEED_IDS.keys():
        ...
        if uniswap_pool:
            uniswap_abi = get_contract_abi(uniswap_pool)
            uniswap_liquidity = get_pool_liquidity(uniswap_pool, uniswap_abi)
        else:
            uniswap_liquidity = None

        if curve_pool:
            curve_abi = get_contract_abi(curve_pool)
            curve_liquidity = get_pool_liquidity(curve_pool, curve_abi)
        else:
            curve_liquidity = None

        print(f"\n[Final Results] {token0}/{token1}")
        print(f"Uniswap Pool: {uniswap_pool}, Liquidity: {uniswap_liquidity}")
        print(f"Curve Pool: {curve_pool}, Liquidity: {curve_liquidity}")
        print("---------------------------------------------------")
