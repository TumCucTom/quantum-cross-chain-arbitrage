...
def execute_trades(selected_edges, expected_profits):
    ...
    def log_trade(trade, expected_profit, txhash, cross_chain_state_validated):
        ...
        trade_log.append(log_entry)  # Store log in memory
        return json.dumps(trade_log, indent=4)  # Return JSON formatted string

    # Execute Flash Loans and Log Trades
    for trade in selected_edges:
        txhash = execute_flash_loan(trade)
        trade_log_json = log_trade(trade, -result.fval, txhash, cross_chain_state_validated=True)

    # Return the final trade log JSON
    print(trade_log_json)

    return json.dumps(trade_log, indent=4)  # Return final trade log as JSON